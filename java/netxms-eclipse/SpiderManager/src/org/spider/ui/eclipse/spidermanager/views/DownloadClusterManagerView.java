...
public class DownloadClusterManagerView extends ViewPart {
	 ...
	 * The constructor.
	 */
	public DownloadClusterManagerView() {
	}
	...
	public void createPartControl(Composite parent) {
		...
		try {
			viewer.setInput(session.getCluster(SpiderCodes.CLUSTER_DOWNLOAD));
		...
		new ConsoleJob("Refresh download cluster list", this,
			...
			protected void jobFailureHandler() {
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						DownloadClusterManagerView.this.getViewSite().getPage()
						.hideView(DownloadClusterManagerView.this);
					}
				});
			}
			...
			protected String getErrorMessage() {
				return "Open download cluster error!";
			}
		}.start();
	}
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				DownloadClusterManagerView.this.fillContextMenu(manager);
			}
		});
	...
	private void makeActions() {
		actAddCluster = new Action("Add new download cluster", 
				Activator.getImageDescriptor("icons/account_add.png")) {
			public void run() {
				addCluster();
			}
		};
		actAddCluster.setToolTipText("Add new download cluster");
		...
		actDeleteCluster.setToolTipText("Delete download cluster");
	...
	{
		final CreateClusterDialog dlg = new CreateClusterDialog(getViewSite().getShell(), "Create new download cluster");
			...
			new ConsoleJob("Create cluster",
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					session.createCluster(dlg.getClusterId(), dlg.getClusterName(), 
							...
							dlg.getPassword(), SpiderCodes.CLUSTER_DOWNLOAD);
				}
	...
	private void modifyCluster()
	{
		...
		if(firstElement instanceof ClusterObject)
		{
			final EditClusterDialog dlg = new EditClusterDialog(getViewSite().getShell(), 
					(ClusterObject)firstElement, "Edit download cluster'");
			if (dlg.open() == Window.OK) {
				new ConsoleJob("Edit download cluster",
						this, Activator.PLUGIN_ID, null) {
					@Override
					protected void runInternal(IProgressMonitor monitor)
							throws Exception {
					...
					@Override
					protected String getErrorMessage() {
						return "Can not edit download cluster";
					}
				}.start();
			}
		}
	...
	private void deleteCluster() throws IOException, NXCException
	{
		...
		if (dialog.open() == SWT.OK) {		
			new ConsoleJob("Delete download cluster", this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
				...
				@Override
				protected String getErrorMessage() {
					return "Can not delete download cluster";
				}
			}.start();
		}
	}

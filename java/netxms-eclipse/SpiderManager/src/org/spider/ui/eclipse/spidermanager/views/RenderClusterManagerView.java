...
public class RenderClusterManagerView extends ViewPart {

	/**
	 * The ID of the view as specified by the extension.
	 */
	public static final String ID = "org.spider.ui.eclipse.spidermanager.views.RenderClusterManager";
	 ...
	 * The constructor.
	 */
	public RenderClusterManagerView() {
	}
	...
	public void createPartControl(Composite parent) {
		...
		try {
			viewer.setInput(session.getCluster(SpiderCodes.CLUSTER_RENDER));
		...
		sessionListener = new SessionListener() {
			...
			public void notificationHandler(final SessionNotification n) {
				if (n.getCode() == SessionNotification.RENDER_CLUSTER_CHANGED) {
		...
		new ConsoleJob("Refresh render cluster list", this,
			...
			protected void jobFailureHandler() {
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						RenderClusterManagerView.this.getViewSite().getPage()
						.hideView(RenderClusterManagerView.this);
					}
				});
			}
			...
			protected String getErrorMessage() {
				return "Open render cluster error!";
			}
		}.start();
	}
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				RenderClusterManagerView.this.fillContextMenu(manager);
			}
		});
	...
	private void makeActions() {
		...
		actAddCluster.setToolTipText("Add new render cluster");

		actEditCluster = new Action("Edit render cluster", 
				Activator.getImageDescriptor("icons/account_edit.png")) {
			public void run() {
				modifyCluster();
			}
		};
		actEditCluster.setToolTipText("Edit render cluster");
		...
		actDeleteCluster.setToolTipText("Delete render cluster");
	...
	{
		final CreateClusterDialog dlg = new CreateClusterDialog(getViewSite().getShell(), "Create new render cluster");
		if (dlg.open() == Window.OK) {
			new ConsoleJob("Create render cluster",
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					session.createCluster(dlg.getClusterId(), dlg.getClusterName(), 
							...
							dlg.getPassword(), SpiderCodes.CLUSTER_RENDER);
				}

				@Override
				protected String getErrorMessage() {
					return "Can not render cluster";
				}
			}.start();
		}
	}
	...
	private void modifyCluster()
	{
		...
		if(firstElement instanceof ClusterObject)
		{
			final EditClusterDialog dlg = new EditClusterDialog(getViewSite().getShell(), 
					(ClusterObject)firstElement, "Edit render cluster");
			if (dlg.open() == Window.OK) {
				new ConsoleJob("Delete render cluster", this,
						Activator.PLUGIN_ID, null) {
					@Override
					protected void runInternal(IProgressMonitor monitor)
							throws Exception {
					...
					@Override
					protected String getErrorMessage() {
						return "Can not delete render cluster";
					}
				}.start();
			}
		}
